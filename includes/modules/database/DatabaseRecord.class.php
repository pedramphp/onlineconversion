...
class DatabaseRecordLoader {
	...
	public function __call($name, $arguments) {
		...
		$loadRegRule 	= "/^Load(?P<DoSingle>Single)?_(?P<table>[$validChars]+)$/";
		$createRegRule 	= "/^Create_(?P<table>[$validChars]+)$/";
		$loadIdRegRule 	= "/^LoadId_(?P<table>[$validChars]+)$/";
	...
	private function ShapreArguments_ForCreate(array $args) {
		...
		switch(count($args)) {
			...
			case 1:			if 		( is_array($args[0]) ) 	{ $initialValues = $args[0]; }
							elseif 	( $tableIsSet === true ){ $database = $args[0]; }
							else{ $table = $args[0]; }
							break;
